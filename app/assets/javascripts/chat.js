        ...
        checkInputKey: function (event, chatboxtextarea, conversation_id) {
            if (event.keyCode == 13 && event.shiftKey == 0) {
                ...
                if (message != '') {
                    ...
                    var valuesToSubmit = $('#conversation_form_' + conversation_id).serialize();
                    $.ajax({
                        type: "POST",
                        url: $('#conversation_form_' + conversation_id).attr('action'), //sumbits it to the given url of the form
                        headers: {
